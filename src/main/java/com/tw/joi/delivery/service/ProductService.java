...
public class ProductService {
  ...
  /**
   * Returns a product for a given outlet.
   *
        ...
        .orElseThrow(
            () ->
                new NotFoundException(
                    "Product not found for outletId=%s and productId=%s"
                        .formatted(outletId, productId)));
  }

  /**
   * Returns all products for a given store.
   *
   * @param storeId store identifier
   * @return list of products
   */
  public List<GroceryProduct> getProductsByStoreId(String storeId) {
    return products.stream().filter(product -> isProductInStore(product, storeId)).toList();
  }

  private boolean isSameProductInStore(
      GroceryProduct groceryProduct, String productId, String outletId) {
